...
#    net = Mininet( topo=topo)
    ...
    net.start()
    sleep(10)
    print "Testing bandwidth between (h1,h3) and (h1,h5)"
    h1, h2, h3, h4, h5, h6, h7, h8 = net.get( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'h7','h8' )
    # 1st server-client pair
    h1.sendCmd('iperf -s -u')
    h3.sendCmd('iperf -c 10.0.0.1 -u -b 10K')
    h5.sendCmd('iperf -c 10.0.0.1 -u -b 10K')
    sleep(14)
    print "Testing bandwidth between (h8,h2) and (h8,h6)"
    # 2nd server-client pair
    h8.sendCmd('iperf -s -u')
    h2.sendCmd('iperf -c 10.0.0.8 -u -b 10K')
    h6.sendCmd('iperf -c 10.0.0.8 -u -b 10K')
    sleep(14)
    CLI( net )
    net.stop()
