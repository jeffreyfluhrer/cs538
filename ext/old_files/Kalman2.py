...
log = core.getLogger()
# Using these as global variables but will clean this up once I get warmed up with Python
#web_bytes = 0
#web_flows = 0
#web_packet = 0

class Kalman2 (object): 
...
#       if f.match.nw_dst == IPAddr("10.0.0.1"):
    ...
    log.info("Flow traffic from %s: %s bytes (%s packets) over %s flows", 
      dpidToStr(event.connection.dpid), self.web_bytes, self.web_packet, self.web_flows)
