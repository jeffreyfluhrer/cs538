...
import time
# 2.  Import Kalman4 for Monitor program
from Kalman4 import *
...
class l3_switch (EventMixin):
  def __init__ (self, fakeways = [], arp_for_unknowns = False, wide = False, output="/home/jeffrey/o2", flows = [], ss=True):
    ...
    # 3.  Adding Monitor program by instaniating class and registering object
    #print("This is the bool " + str(ss))
    self.monitor = Monitor(output=output, flows=flows,ss=ss)
    core.register("Monitor", self.monitor)

    # TODO:  look into what this does exactly, I think it connects methods to events
    ...
    if isinstance(packet.next, ipv4):
      ...
      if packet.next.srcip in self.arpTable[dpid]:
        if self.arpTable[dpid][packet.next.srcip] != (inport, packet.src):
          ...
          print("%i %i RE-learned %s", dpid,inport,packet.next.srcip)
          if self.wide:
      ...
      if dstaddr in self.arpTable[dpid]:
        ...
        if prt == inport:
          ...
          #          % (dpid, inport, packet.next.srcip, dstaddr, prt))
          # 4.  Add route additions here
          # Get the switch number (just use dpid)
          # Get the source id (get the last number in address -- I good up to nine hosts_
          srcId = str(packet.next.srcip)
          srcId = srcId[-1]
          # Get the dest id
          destId = str(dstaddr)
          destId = destId[-1]
          # This adds the route to the Kalman filter
          self.monitor.filter.AddRouteForRouteMatrix(switchNo=dpid, HostSrc=int(srcId), HostDst=int(destId))
...
# 1.  adding timer for use by monitor program
...
def launch (fakeways="", arp_for_unknowns=None, wide=False, output="/home/jeffrey/minitest/output2", flows="", ss="True"):
  from pox.lib.recoco import Timer
  ...
  ss = str_to_bool(ss)
  #print("This is the bool " + str(ss))
  core.registerNew(l3_switch, fakeways, arp_for_unknowns, wide, output, flows, ss)
